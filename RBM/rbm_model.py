...
class RBM(object):
    def __init__(self, visible_dim, hidden_dim, learning_rate, number_iter):

        self._graph = tf.Graph()

        # initialize graphs
        with self._graph.as_default():
            self._num_iter = number_iter
            ...
            self._visible_cdstates = tf.Variable(tf.zeros([1, visible_dim], tf.float32, name='visible_bises'))
            ...
            self._input_sample = tf.placeholder(tf.float32, [visible_dim], name='imput_sample')
    ...
    def train(self, input_vects):
        for iter_num in range(self._num_iter):
            for input_vect in input_vects:
                self._sess.run(self._training, feed_dict={self._input_sample, input_vect})

    def calculate_dist(self, probability):
